...
class SplitEngine {
    ...
    // ========================
    // uodId数据完整性校验
    // 原理：拆票只新增列+分组排序，原始列数据不应有任何变化
    // 做法：用拆票前的快照（只有原始列）逐单元格比对拆票后的结果
    // ========================
    validateByUodId(originalSnapshot, resultRows) {
        ...
        const report = {
            ...
            resultCount: 0,
            countMatch: true,
            missingIds: [],      // 原始有但结果没有的uodId（行丢失）
            extraIds: [],        // 结果有但原始没有的uodId（凭空多出行）
            duplicateIds: [],    // 结果中重复的uodId
            cellMismatches: [],  // 同uodId但某列值不一致（数据错位）
            totalMismatches: 0
        };

        // 构建快照映射: uodId -> 行（快照只有原始列，不含分票编号/分票等新增列）
        const snapshotMap = {};
        const snapshotIds = [];
        for (const row of originalSnapshot) {
            const uid = String(this.getCellValue(row, idColumn)).trim();
            if (!uid || uid === 'undefined') continue;
            snapshotMap[uid] = row;
            snapshotIds.push(uid);
        }
        report.originalCount = snapshotIds.length;

        // 构建结果映射: uodId -> 行（跳过 null 空行分隔符）
        const resultMap = {};
        const resultIds = [];
        ...
        // 检查1: 行数匹配
        report.countMatch = (snapshotIds.length === resultIds.length);
        if (!report.countMatch) report.passed = false;

        // 检查2: 丢失的uodId（快照有，结果没有 → 行被丢失）
        for (const id of snapshotIds) {
            if (!resultMap[id]) report.missingIds.push(id);
        }
        if (report.missingIds.length > 0) report.passed = false;

        // 检查3: 多余的uodId（结果有，快照没有 → 凭空多出）
        for (const id of resultIds) {
            if (!snapshotMap[id]) report.extraIds.push(id);
        }
        ...
        // 检查5: 逐单元格比对
        // 快照行只有原始列，遍历快照的 keys 就是遍历所有原始列
        // 新增列（分票编号/分票等）不在快照中，天然跳过
        const comparedUids = new Set();
        for (const row of resultRows) {
            if (row === null) continue;
            const uid = String(this.getCellValue(row, idColumn)).trim();
            if (!uid || comparedUids.has(uid)) continue;
            comparedUids.add(uid);

            const origRow = snapshotMap[uid];
            if (!origRow) continue;

            // 遍历快照行的所有 key（= 原始列），逐单元格比对
            for (const col of Object.keys(origRow)) {
                const origVal = origRow[col] != null ? String(origRow[col]).trim() : '';
                const resultVal = row[col] != null ? String(row[col]).trim() : '';

                if (origVal !== resultVal) {
                    report.cellMismatches.push({
                        uodId: uid,
                        column: col,
        ...
        if (report.cellMismatches.length > 0) report.passed = false;
        report.totalMismatches = report.missingIds.length + report.extraIds.length
            + report.duplicateIds.length + report.cellMismatches.length;
